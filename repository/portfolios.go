...
func PortfoliosExt() (entity.PortfoliosExt, error) {
	...
	err := db.Connection().Select(&portfolios, "SELECT portfolio_id, name, currency, cache_value, gain_of_sold_shares, commision, tax, gain_of_owned_shares, estimated_gain, estimated_gain_costs_inc, estimated_value FROM portfolios_ext")
